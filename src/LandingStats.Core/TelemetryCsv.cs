...
public static class TelemetryCsv
{
    ...
    private const int LegacyContactPointCount = 64;
    ...
    public static readonly string DiagnosticWithoutPositionHeader = BuildHeader(includeContactPosition: false, includeBlackBoxChannels: false, includeControlDiagnostics: false, LegacyContactPointCount);

    public static readonly string PreviousHeader = BuildHeader(includeContactPosition: true, includeBlackBoxChannels: false, includeControlDiagnostics: false, LegacyContactPointCount);

    public static readonly string BlackBoxV3Header = BuildHeader(includeContactPosition: true, includeBlackBoxChannels: true, includeControlDiagnostics: false, LegacyContactPointCount);

    public static readonly string V4Header = BuildHeader(includeContactPosition: true, includeBlackBoxChannels: true, includeControlDiagnostics: true, LegacyContactPointCount);
    ...
    public static IReadOnlyList<TelemetrySample> ReadFile(string path)
    {
        ...
        if (!string.Equals(header, Header, StringComparison.Ordinal) &&
            !string.Equals(header, V4Header, StringComparison.Ordinal) &&
    ...
    public static bool TryParse(string line, out TelemetrySample sample)
    {
        ...
        if ((!isLegacy && !isDiagnosticWithoutPosition && !isPrevious && !isBlackBoxV3 && !isV4) ||
        ...
        if (!isLegacy)
        {
            ...
            if (isBlackBoxV3 || isV4 || isCurrent)
            {
            ...
            var sourceContactPointCount = isCurrent
                ? TelemetrySample.CapturedContactPointCount
                : LegacyContactPointCount;
            for (var contactIndex = 0; contactIndex < sourceContactPointCount; contactIndex++)
            {
                ...
                if (contactIndex < sample.ContactPointCompression.Length)
                {
            ...
            if (!isDiagnosticWithoutPosition)
            {
                for (var contactIndex = 0; contactIndex < sourceContactPointCount; contactIndex++)
                {
                    ...
                    if (contactIndex < sample.ContactPointPosition.Length)
                    {
            ...
            for (var contactIndex = 0; contactIndex < sourceContactPointCount; contactIndex++)
            {
                ...
                if (contactIndex < sample.ContactPointOnGround.Length)
                {
    ...
    private static bool TryParseCompact(string[] values, out TelemetrySample sample)
    {
        sample = new TelemetrySample();
        var index = 0;
        if (!long.TryParse(values[index++], NumberStyles.Integer, Invariant, out var sequence) ||
            !TryNumber(values[index++], out var hostElapsed) ||
            !TryNumber(values[index++], out var simulationTime) ||
            !TryNumber(values[index++], out var simulationDelta) ||
            !TryBoolean(values[index++], out var onGround) ||
            !TryBoolean(values[index++], out var motionSimulation) ||
            !TryNumber(values[index++], out var touchdownVelocity) ||
            !TryNumber(values[index++], out var verticalSpeed) ||
            !TryNumber(values[index++], out var worldY) ||
            !TryNumber(values[index++], out var gForce) ||
            !TryNumber(values[index++], out var agl) ||
            !TryNumber(values[index++], out var pitch) ||
            !TryNumber(values[index++], out var bank) ||
            !TryNumber(values[index++], out var latitude) ||
            !TryNumber(values[index++], out var longitude) ||
            !TryNumber(values[index++], out var indicatedAirspeed) ||
            !TryNumber(values[index++], out var groundSpeed) ||
            !TryNumber(values[index++], out var planeAltitude) ||
            !TryNumber(values[index++], out var groundAltitude) ||
            !TryNumber(values[index++], out var rotationX) ||
            !TryNumber(values[index++], out var rotationY) ||
            !TryNumber(values[index++], out var rotationZ) ||
            ...
            !TryNumber(values[index++], out var totalWeight) ||
            !TryNumber(values[index++], out var cgPercent) ||
            !int.TryParse(values[index++], NumberStyles.Integer, Invariant, out var numberOfEngines) ||
            !TryNumber(values[index++], out var pilotRoll) ||
            !TryNumber(values[index++], out var pilotPitch) ||
            !TryNumber(values[index++], out var rudderPedal))
        {
        ...
        for (var engineIndex = 0; engineIndex < TelemetrySample.CapturedEngineCount; engineIndex++)
        {
            if (!TryNumber(values[index++], out var value))
            {
                return false;
            }

            sample.EngineThrottlePercent[engineIndex] = value;
        }
        ...
        for (var engineIndex = 0; engineIndex < TelemetrySample.CapturedEngineCount; engineIndex++)
        {
            ...
            sample.EngineN1Percent[engineIndex] = value;
        }
        ...
        for (var engineIndex = 0; engineIndex < TelemetrySample.CapturedEngineCount; engineIndex++)
        {
            ...
            sample.EngineRpm[engineIndex] = value;
        }
        ...
        for (var engineIndex = 0; engineIndex < TelemetrySample.CapturedEngineCount; engineIndex++)
        {
            ...
            sample.EngineReversePercent[engineIndex] = value;
        }
        ...
        for (var contactIndex = 0; contactIndex < TelemetrySample.CapturedContactPointCount; contactIndex++)
        {
            ...
            sample.ContactPointCompression[contactIndex] = value;
        }
        ...
        for (var contactIndex = 0; contactIndex < TelemetrySample.CapturedContactPointCount; contactIndex++)
        {
            ...
            sample.ContactPointPosition[contactIndex] = value;
        }

        for (var contactIndex = 0; contactIndex < TelemetrySample.CapturedContactPointCount; contactIndex++)
        {
            if (!TryBoolean(values[index++], out var value))
            {
                return false;
            }

            sample.ContactPointOnGround[contactIndex] = value;
        }
    ...
    private static string BuildCompactHeader()
    {
        var columns = new List<string>
        {
            ...
            "on_ground",
            "motion_simulation",
            "touchdown_normal_velocity_fps",
            "vertical_speed_fps",
            "velocity_world_y_fps",
            "g_force",
            "agl_ft",
            "pitch_deg",
            "bank_deg",
            ...
            "cg_percent",
            "number_of_engines",
            "pilot_roll_input_percent",
            "pilot_pitch_input_percent",
            "rudder_pedal_input_percent",
        };
    ...
    private static string BuildHeader(
        bool includeContactPosition,
        bool includeBlackBoxChannels,
        bool includeControlDiagnostics,
        int contactPointCount)
    {
        ...
        for (var index = 0; index < contactPointCount; index++)
        {
            columns.Add($"contact_{index}_compression");
        }
